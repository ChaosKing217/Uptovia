...
// Helper function to check if user can create monitors
async function canUserCreateMonitor(userId) {
    try {
        ...
        // Admins can always create monitors
        if (user.is_admin) {
            return { allowed: true };
        }
...
router.post('/', async (req, res) => {
    try {
        ...
        // Check if user can create monitors (email verification required for non-admins)
        const permission = await canUserCreateMonitor(req.userId);
